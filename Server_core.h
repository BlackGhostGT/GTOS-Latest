...
#define REGISTRATION
const char* ip = "localhost"; /*5.20.120.177*/
const char* user = "root";
const char* pass = "@Almantas2004";
const char* database = "server";
MYSQL* conn;
MYSQL_RES* res;
/*naudojami tik threadams*/
MYSQL* WorldLoader;
MYSQL_RES* WorldLoaderResponse;
MYSQL* WorldSaver;
MYSQL_RES* WorldSaverResponse;
MYSQL* TimerHandler;
MYSQL_RES* TimerHandlerResponse;
...
string CacheIP = "gtosprivate.com";
...
inline void SendConsole(const string text, const string type) {
	...
	/*if (type == "ERROR") { 
		...
		threads.push_back(std::thread(SendDiscordNotification, superrealtext, "https://discord.com/api/webhooks/785830757683560499/ELi1nBTgAZQHUCnWEYbfg9q0zxnAVn9OXY0CztujO0qNxFAyIOzcEBNhEDf-stkbZT6g"));
		return;
	}
	if (type == "MUTE") {
		const auto superrealtext = space2underscore("[" + to_string(Hour) + ":" + to_string(Min) + ":" + to_string(Sec) + " " + type + """]: " + text);
		threads.push_back(std::thread(SendDiscordNotification, superrealtext, "https://discord.com/api/webhooks/784788699460141077/osi25vM1DzNgfWu1C-qnYAWjmKqaIuZLf47HbAevewgzJarYB1F0pXoveOeEkWrFfrlc"));
		return;
	}
	if (type == "CURSE") {
		const auto superrealtext = space2underscore("[" + to_string(Hour) + ":" + to_string(Min) + ":" + to_string(Sec) + " " + type + """]: " + text);
		threads.push_back(std::thread(SendDiscordNotification, superrealtext, "https://discord.com/api/webhooks/784788477949640735/yEeUIJJJ2YCY8fE3GnDHxJiCl82No8Z0WzPeGt7C7kxzVRcpeV5Mq4-cAG3mcYZ_RxaV"));
		return;
	}
	if (type == "BAN") {
		const auto superrealtext = space2underscore("[" + to_string(Hour) + ":" + to_string(Min) + ":" + to_string(Sec) + " " + type + """]: " + text);
		threads.push_back(std::thread(SendDiscordNotification, superrealtext, "https://discord.com/api/webhooks/784788480390856754/Lp1WnbPZPGKoWAzD_Mnx6M_CDrK7yKNxD2TOyK2TZd45SX0KA3bo3eLlHYSt5jZio-GV"));
		return;
	}
	if (type == "CHAT" && text.find("@everyone") == string::npos && text.find("@here") == string::npos && text.find("http") == string::npos) {
		threads.push_back(std::thread(SendDiscordNotification, text, "https://discord.com/api/webhooks/784788294701023250/KrtH4VK3PzbkOGB9zBSuY5oBd_ZeTCZlZvoWaIGHwUCxYlN8LRWvFHzRPvfYaIRVWjhH"));
	}*/
}
...
bool WorldExist(string name) {
	MYSQL_ROW WorldRows;
	string query = "SELECT EXISTS(SELECT 1 FROM WorldDatabase WHERE name ='" + name + "' LIMIT 1)";
